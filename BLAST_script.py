...
def blastx(inp_file, output_file):
    ...
    cmd = "blastx -query "+inp_file+" -db nr -out "+output_file+" -evalue 10e-10 -culling_limit 1 -outfmt '6 qseqid sseqid slen pident length mismatch gapopen qstart qend sstart send evalue bitscore staxids sscinames sskingdoms' -seg yes"
    write_batch_script(inp_file.strip(".fa")+"_runBLAST.sh", 1, 1000, "general" ,16000, cmd)
